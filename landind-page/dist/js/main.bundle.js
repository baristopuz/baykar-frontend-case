...
;// CONCATENATED MODULE: ./src/js/partials/swiper-sliders.js
...
function initSliders() {
  ...
  sliderElements.forEach(function (sliderElement, index) {
    var data = sliderElement.getAttribute('data-swiper') || {};
    var dataTemplate = sliderElement.getAttribute('data-swiper-template') || null;
    var swiperType = sliderElement.getAttribute('data-swiper-type') || null;
    ...
    var options = _objectSpread(_objectSpread({}, defaultOptions), dataOptions);
  ...
  sliders.thumb.forEach(function (item, index) {
    var swiper = new Swiper(item.element, item.options);
    initializedSliders.thumb.push({
      swiper: swiper,
      title: item.title
    });
  });
  sliders.normal.forEach(function (item, index) {
    if (item.options.thumbSwiperTitle) {
      var thumbSwiper = initializedSliders.thumb.find(function (thumbItem) {
        return thumbItem.title === item.options.thumbSwiperTitle;
      });
      if (thumbSwiper) {
        item.options.thumbs = {
    ...
    function smoothScrollID() {
      ...
      scrollElements.forEach(function (scrollElement) {
        scrollElement.addEventListener('click', function (item) {
          var element = document.querySelector(scrollElement.getAttribute('scroll-el')); // Replace 'myElement' with the actual ID of the target element.
          var offset = element.offsetTop - 64; // Get the vertical offset of the element from the top.
          window.scrollTo({
            top: offset,
